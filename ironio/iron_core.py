...
class IronClient:
    ...
    def __init__(self, name, version, product, host=None, project_id=None,
        ...
        config = configFromArgs(
            config, host=host, project_id=project_id,
        ...
        for field in required_fields:
            if config[field] is None:
                raise ValueError("No %s set. %s is a required field." % (
                    field, field))
        ...
        if self.protocol == "https" and self.port != 443:
            raise ValueError("Invalid port (%s) for an HTTPS request. Want %s." % (self.port, 443))
